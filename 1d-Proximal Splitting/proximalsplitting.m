function [cost, f_path, m_path, Pi_final, x_f, T_x] = dynamic_optimal_transport_1D(Nx, Nt, mu1, mu2, xa, xb, ya, yb)
    ...
    % ---------------------------------------------------------
    % Dynamic Map T(x) = x + v(x, t=0), evaluated at xhat
    % ---------------------------------------------------------
    % The Benamou-Brenier geodesic satisfies T_t(x) = (1-t)x + t*T(x), so the
    % velocity field is constant along characteristics and the optimal map
    % is recovered in closed form from the t=0 velocity alone:
    %   T(x) = x + v(x, 0).
    %
    % IMPORTANT: do NOT use the centered-grid v_c = mc./fc at its first slab.
    % fc(:,1) = 0.5*(f_path(:,1) + f_path(:,2)) is time-AVERAGED and therefore
    % already blends in the solved density at t=dt, contaminating v(.,0) with
    % an O(dt) error. Instead use the EXACT boundary data f(.,0) = mu1_vec
    % (imposed, not solved) together with the momentum's first staggered time
    % slab m_path(:,1), averaged only in SPACE (never in time) to land on
    % cell centers:
    m0_centered = 0.5 * (m_path(1:end-1, 1) + m_path(2:end, 1)); % [Nx,1], space-avg only, t~0
    
    tol  = 1e-4 * max(mu1_vec); ind  = find(mu1_vec > tol);
    v0 = m0_centered(ind) ./ mu1_vec(ind); x_f = x_f(ind);
    % v0 = m0_centered./ mu1_vec;
    
    % Note: with xa=ya, xb=yb (shared source/target domain), x_min=xa, x_max=xb,
    % so xhat and x_f are the SAME grid (both Nx equally-spaced midpoints of
    % [xa,xb]) up to floating-point roundoff. No interpolation is needed:
    % v0, already on x_f, can be read off directly as the velocity at xhat.
    T_x = x_f + v0;
